...
function Home() {
    ...
    const handleConfirmBooking = async () => {
        ...
        try {
            ...
            const response = await confirmBooking({ eventId: selectedEvent._id, seats: selectedSeats, userId, bookingId }, { "Content-Type": "application/json", Authorization: `Bearer ${token}` })
            if (response.status === 200) {
                navigate("/booking-success", { state: { event: selectedEvent, seats: selectedSeats, bookingId } })
                closeModal();
            }
        } catch (err) {
                ...
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    <section className="text-center py-16">
                        <h1 className="text-4xl md:text-5xl font-bold leading-tight">
                            Find the events and
                            <br />
                            venues in the town! Right here
                        </h1>
                        <div className="flex justify-center items-center gap-4 mt-10">
                            <button className="px-8 py-3 rounded-full bg-zinc-900 border border-zinc-700 font-medium shadow-lg">
                                Popular Events
                            </button>
                                ...
                                <div className="grid grid-cols-5 gap-2">
                                    {seats.map((seat) => {
                                        ...
                                        return (
                                            <button
                                                key={seat.seatNo}
                                                disabled={seat.status !== "available" || isReserved}
